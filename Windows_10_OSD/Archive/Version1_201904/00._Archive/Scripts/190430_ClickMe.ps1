...
Start-Transcript -Path "$PSScriptRoot\$(Get-Date -Format "yyMMdd-HHmmss-ffffff")_$($MyInvocation.MyCommand.Name).log"
	...
	if (Test-Path -Path $MountPath) {
		...
		try {
			...
			if ($MountedImages = Get-WindowsImage -Mounted | Where-Object { (Join-Path $_.Path '') -eq (Join-Path $MountPath '') }) {
				Write-Host "-Removing: [$($MountedImages.Path) | $($MountedImages.ImagePath) (Index: $($MountedImages.ImageIndex))] with -Discard!" -ForegroundColor Yellow
				$null = Dismount-WindowsImage -Path $MountPath -Discard -Verbose
			}

			<#
			# Remove ALL mounted folders
			if($AllMountPaths = Dism /English /Get-MountedWimInfo | Select-String "Mount Dir : ") {
				Write-Host "Mount path(s) found" -ForegroundColor Yellow

				$AllMountPaths | ForEach-Object {
					$thisMountPath = ($_ | Select-Object -Property Line | Sort-Object -Descending -Property Line | Select-Object -First 1).Line -replace "Mount Dir : "

					# Only removing specified path
					if((Join-Path $thisMountPath '') -eq (Join-Path $MountPath '')) {
						Write-Host "-Unmounting `"$thisMountPath`"" -ForegroundColor Yellow
						Dism /English /Unmount-Wim /MountDir:$thisMountPath /Commit
						Dism /English /Unmount-Wim /MountDir:$thisMountPath /Discard # Just in case we cannot save.
					} else {
						Write-Host "Not our business, ignoring path `"$thisMountPath`"" -ForegroundColor Yellow
					}
				}
			}
			#>

			# maybe i should also check for "Image File"
			<#
			Mounted images:
			Mount Dir : C:\ISOMount
			Image File : C:\tmp\w10_custom_image\w10\IDonkIDonk.wim
			Image Index : 1
...
if (!(Test-Path -Path $UUPDownloader -ErrorAction SilentlyContinue)) {
	...
	if ($ISOFiles) {
		$ISOFiles | ForEach-Object {
			...
			$InstallImage = Get-Item -Path "$IMAGE_DriveLetter`:\sources\install.*"
			...
			$CopiedImageFullName = "$PSScriptRoot\$CopiedImageFileName"
			...
			if (!$OnlyEnterprise -and !$DoNotRemoveN) {
				...
				Dism /English /Get-ImageInfo /ImageFile:$CopiedImageFullName | Select-String "^Name :.*?( N).*?$" -Context 1, 0 | ForEach-Object {
					...
					Remove-WindowsImage -ImagePath $CopiedImageFullName -Name "$ReplaceThisIndexName" -CheckIntegrity
				}
			}
			...
			if (![String]::IsNullOrEmpty($Selection)) {
				$Selection.Split(",") | ForEach-Object {
					...
					try {
						...
						# e.g.: If edition #4 does not exist, how can we change edition #5
						return
					}
				}
			}
			...
			if (!$SkipImageCleanUp -or ($WindowsUpdateFiles -and !$SkipUpdateFolder)) {
				...
				$Indizies | ForEach-Object {
					$Count++
					Write-Host "==============================="
					Write-Host "Processing Image #$Count/$($Indizies.Count)"
					Write-Host "==============================="

					Write-Host "Mounting Image"
					try {
						# Replace index number if we want to insert (N)o-Media-Version!
						$null = Mount-WindowsImage -Path $MountPath -ImagePath $CopiedImageFullName -Index $Count -Verbose
					}
					catch {
						Write-Host "Error: $($_.Exception.Message)" -ForegroundColor Red
					}
					...
					if ($Version -and ($Version -ne [System.Version]"0.0.0.0") -and $WindowsUpdateFiles -and !$SkipUpdateFolder) {
						...
						$WindowsUpdateFiles | ForEach-Object {
							...
							Get-ChildItem -Path "$KBExtractFolder" -Filter "*.cab" -Recurse | Where-Object { $_.Name -notmatch "WSUSSCAN.cab" } | ForEach-Object {
								...
								try {
									(Get-WindowsPackage -Path $MountPath -PackagePath $_.FullName).PackageName
					...
					if (!$SkipImageCleanUp) {
						...
						# NOBODY SHOULD USE SMB1 ANYWAY!!
						$null = Disable-WindowsOptionalFeature -Path $MountPath -FeatureName SMB1Protocol
						...
						$featuresAdd | ForEach-Object {
							...
							if ($null = Get-WindowsOptionalFeature -Path $MountPath -FeatureName $tFeature) {
								...
								try {
									...
									$null = Enable-WindowsOptionalFeature -Path $MountPath -FeatureName $tFeature -Source "$IMAGE_DriveLetter`:\sources\sxs" # Source only when not found?
									$Success = $true
								}
								catch {
								...
								# Should never be run as the PowerShell-Command does evenrything needed.
						...
						if (Test-Path -Path "$MountPath\Windows\System32\OneDriveSetup.exe" -ErrorAction SilentlyContinue) {
							$null = Rename-Item -Path "$MountPath\Windows\System32\OneDriveSetup.exe" -NewName "onedrivesetup.bck" -Verbose
						}
						if (Test-Path -Path "$MountPath\Windows\SysWOW64\OneDriveSetup.exe" -ErrorAction SilentlyContinue) {
							$null = Rename-Item -Path "$MountPath\Windows\SysWOW64\OneDriveSetup.exe" -NewName "onedrivesetup.bck" -Verbose
						}
						...
						foreach ($app in $apps) {
							$Count++
							Write-Host "[$Count/$($Apps.Count)] Push removal of `"$app`"" -ForegroundColor DarkGray
						...
						if (Test-Path -Path "$MountPath\Users\default\ntuser.dat") {
							...
							if (Test-Path -Path "HKLM:\OFDEFUSR") {
								...
								Set-RegistryValue -Path "HKLM:\OFDEFUSR\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "HideFileExt" -Value 1 # Show File Extensions
						...
						$Max = 30
						do {
							$Result = Dism /English /Image:$MountPath /Cleanup-Image /StartComponentCleanup /ResetBase | Select-String "pending"
							if ($Result) {
								$Count++
								Write-Host "Waiting for pending dism command"
						...
						#$null = Dism /English /Image:$MountPath /Cleanup-Image /StartComponentCleanup /ResetBase

						# Possible Changing Stuff:
						# Final touch: C:\Win10-Mount\Windows\Setup\Scripts\SetupComplete.cmd
			...
			if (!$DoNotCreateISO) {
				...
				if ( $ADK_Exists -and $Robocopy_Exists ) {
					Write-Host "Creating ISO-File in C:\$($IMAGE_DriveInformation.FileSystemLabel).iso" -ForegroundColor Cyan

					# /MIR works, if nothing is left ;-)
					ROBOCOPY "$IMAGE_DriveLetter`:\" $MountPath /E /R:1 /W:10 /TEE /Log+:"$(Get-Date -Format "yyMMdd-HHmmss-ffffff")_robo.log"

					# Instert WIM into Image
					Copy-Item -Path $CopiedImageFullName -Destination "$MountPath\sources\install.wim" -Force -PassThru
