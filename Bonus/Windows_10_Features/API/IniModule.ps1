...
#region ExportINI
# Changed: I NEED SPACES FOR THE CONVERTER CONFIG...
    ...
    Process {
        ...
        foreach ($i in $InputObject.keys) {
            if (!($($InputObject[$i].GetType().Name) -eq "Hashtable")) {
                #No Sections
                Write-Verbose "$($MyInvocation.MyCommand.Name):: Writing key (hash): $i"
                Add-Content -Path $outFile -Value "$i=$($InputObject[$i])" -Encoding $Encoding
                #Add-Content -Path $outFile -Value "$("$i".PadRight(13, " "))=$($InputObject[$i])" -Encoding $Encoding
            }
            else {
                #Sections
                ...
                Foreach ($j in $($InputObject[$i].keys | Sort-Object)) {
                    if ($j -match "^Comment[\d]+") {
                        ...
                        #Add-Content -Path $outFile -Value "$("$j".PadRight(13, " "))=$($InputObject[$i][$j])" -Encoding $Encoding
                    }
                }
