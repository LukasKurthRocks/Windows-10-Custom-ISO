	...
	if (Test-Path -Path $MountPath) {
		...
		try {
			...
			$MountedImages | ForEach-Object {
				Write-Host "-Removing: [$($_.Path) | $($_.ImagePath) (Index: $($_.ImageIndex))] with -Discard!" -ForegroundColor Yellow
			...
			if (!$SkipImageCleanUp -or ($WindowsUpdateFiles -and !$SkipUpdateFolder)) {
				...
				$Indizies | ForEach-Object {
					...
					if (!$SkipImageCleanUp) {
						...
						# VLSC is getting FOD later, i hope.
						if (Test-Path -Path "$PSScriptRoot\FOD\RSAT\$($Version.Build)*") {
							Write-Host "+Add RSAT" -ForegroundColor Yellow
							$FOD_Folder = Get-Item -Path "$PSScriptRoot\FOD\RSAT\$($Version.Build)*" | Where-Object { $_.PSIsContainer } | Select-Object -First 1
