					...
					if (!$SkipImageCleanUp) {
						...
						if (!($SkipFOD)) {
							...
							if (Test-Path -Path "$PSScriptRoot\FOD\RSAT\$($Version.Build)*\$WIMArchitecture") {
								...
								# TODO: params with validation check!
								$SelectedFODsRegEx = @("RSAT")
								$SelectedFODsRegEx | ForEach-Object {
									$thisFeature = $_
									
									$FOD_COUNT = 0
									$FOD_CAP = Get-WindowsCapability -Path $MountPath | Where-Object { $_.Name -match "$thisFeature" -and $_.State -eq "NotPresent" }
									if ($FOD_CAP) {
										$FOD_CAP | ForEach-Object {
											$FOD_COUNT++

											# It's possible to do this in a one-liner.
											# Verbose of these DISM-like commands is not so useful.
											Write-Verbose "[$FOD_COUNT/$($FOD_CAP.Count)] Feature: $($_.Name)" -Verbose
											$null = $_ | Add-WindowsCapability -Path $MountPath -Source $FOD_Folder.FullName
										}
									}
									else {
										Write-Host "No feature with name `"$thisFeature`" found." -ForegroundColor Red
									}
								}
								...
								$RSAT_CAP | ForEach-Object {
									...
									# It's possible to do this in a one-liner.
									# Verbose of these DISM-like commands is not so useful.
									Write-Verbose "[$RSAT_COUNT/$($RSAT_CAP.Count)] Feature: $($_.Name)" -Verbose
									$null = $_ | Add-WindowsCapability -Path $MountPath -Source $FOD_Folder.FullName
								}
								#>
							}
						...
						if (!$SkipStartLayout) {
							...
							if (Test-Path -Path $LayoutFile) {
								...
								$null = Copy-Item -LiteralPath $LayoutFile -Destination "C:\ISOMount\Users\Default\AppData\Local\Microsoft\Windows\Shell\LayoutModification.xml" -Force -Verbose
							}
							else {
								Write-Host "File `"`" not found. No start menu to apply." -ForegroundColor Red
							}
						}
						#endregion StartMenu
